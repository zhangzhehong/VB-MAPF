...
# function: vein-based pattern formation where slope k = pi/4,y-x=c
# author : zhehong
# date : 2022.6.1
...
file_data_timestep = open('output/VB3_timestep.txt','w')
file_data_max_dis = open('output/VB3_max_dis.txt','w')
file_data_sum_dis = open('output/VB3_sum_dis.txt','w')
...
for n in size_list:
    ...
    for shape_index in range(1,37):
                                ...
                                for target_point in target_list:
                                    ...
                                    if flag[target_point[1]][target_point[0]] == 0:
                                        ...
                                        while target_point[1]-0.5-stride >= exact_coalitions[search_index][0][1]:
                                            if -search_index+(target_point[1]-0.5-stride)>=0 and flag[target_point[1]-0.5-stride][-search_index+(target_point[1]-0.5-stride)] == 1 and move_flag[row_hash[target_point[1]-0.5-stride]] == -1:
                                        ...
                                        if flag[current_point[1]][current_point[0]] == 1:
                                            if current_point[1]-0.5 >= 0 and current_index - (current_point[1]-0.5) >=0 and current_index - (current_point[1]-0.5) < num_cols:
                                                if -search_index+(current_point[1]-0.5)>=0 and  -search_index+(current_point[1]-0.5)<num_cols:
                                                    if -current_index+(current_point[1]-0.5)>=0 and flag[current_point[1]-0.5][-current_index+(current_point[1]-0.5)]==0 and flag[current_point[1]-0.5][-search_index+(current_point[1]-0.5)]==1:
        ...
        while exact != n:
            ...
            max_dis = 0
            for index in index_list:
                if len(target_coalitions[index]) > 0:
